...
def payment_callback(name):
    return render_template('success.html', name='Hi')
...
def insert_record(license_plate,encoded_image):
    ...
    existing_record = InRecord.query.filter_by(license_plate=license_plate).filter(InRecord.entry_time <= datetime.now() - timedelta(minutes=30)).first()
...
def insert_out_record(license_plate):
    ...
    existing_record = InRecord.query.filter_by(license_plate=license_plate).first()

    if existing_record:
        # 如果找到匹配的記錄，更新 exit_time 和 url
        existing_record.exit_time = datetime.utcnow()  # 更新離場時間為當前時間
...
def handle_frame(msg):
    ...
    for box in result.boxes:
        ...
        # 在圖像上顯示車牌號碼
        cv2.putText(frame, f'{plate_number}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
...
@app.route('/ecpay/<license_plate>')
def ecpay_url(license_plate):
    return render_template('ecpay_template.html', html_form=html_form,license_plate=license_plate)
